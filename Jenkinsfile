pipeline {
    ...
    environment {
        ...
        DOCKERHUB_USERNAME = 'mokchhedul'  // Set this directly to your Docker Hub username
    ...
    stages {
        stage('Git Checkout') {
            steps {
                git branch: 'main', url: 'https://github.com/DeveloperMokchhedul/devops.git'
            }
        }
        ...
        stage('Sonar Analysis') {
            steps {
                withSonarQubeEnv('sonar-server') {
                    sh "$SCANNER_HOME/bin/sonar-scanner -Dsonar.projectName=mokchhedul -Dsonar.projectKey=KasmokchhedulKey -Dsonar.java.binaries=target"
                }
            }
        }
